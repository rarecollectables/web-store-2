...
async function getLocationInfo() {
  try {
    // Make sure we're using the standard fetch API
    const fetchFunc = typeof fetch === 'function' ? fetch : window.fetch;
    ...
    const res = await fetchFunc('/.netlify/functions/proxy-ipapi');
    if (!res.ok) {
      let errorText;
      try {
