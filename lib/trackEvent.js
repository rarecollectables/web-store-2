...
async function getLocationInfo() {
  try {
    ...
    if (typeof fetch !== 'function') {
      console.error('getLocationInfo: fetch is not available');
      return { 
        error: true, 
    ...
    // Always use the Netlify function endpoint for production and local
    try {
      const res = await fetch('/.netlify/functions/proxy-ipapi');
      if (!res.ok) {
        let errorText;
        try {
        ...
        return { 
          error: true, 
          message: `Fetch response not OK: ${res.status}`,
          city: null, 
          country_name: null 
        };
      }
      ...
      console.error('getLocationInfo: fetch execution error', fetchErr);
      return { 
        error: true, 
        message: fetchErr.message || 'Error executing fetch',
        city: null, 
        country_name: null 
      };
    }
