...
async function getLocationInfo() {
  ...
  // Hardcoded fallback response to avoid any fetch-related errors
  // This ensures we always return a valid object even if the request fails
  ...
  try {
    // Use a simple XMLHttpRequest to get location data
    return await new Promise((resolve) => {
      try {
        ...
        xhr.onload = function() {
          if (xhr.status >= 200 && xhr.status < 300) {
            try {
              ...
              console.error('getLocationInfo: JSON parse error', e);
              resolve(fallbackResponse);
            }
          } else {
            console.error('getLocationInfo: Bad status code', xhr.status);
            resolve(fallbackResponse);
          }
        };
        ...
        xhr.onerror = function() {
          console.error('getLocationInfo: Network error');
          resolve(fallbackResponse);
        };
        
        // Handle timeouts
        xhr.ontimeout = function() {
          console.error('getLocationInfo: Request timeout');
          resolve(fallbackResponse);
        };
        ...
        console.error('getLocationInfo: XHR creation error', err);
        resolve(fallbackResponse);
      }
    });
  } catch (outerErr) {
    console.error('getLocationInfo: Outer promise error', outerErr);
